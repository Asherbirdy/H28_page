...
<template>
  <div v-if="loading" class="fixed inset-0 flex-center z-50"
    :style="{ background: 'radial-gradient(ellipse at center, #FBF6EC 0%, #F0E6D2 100%)' }">
    <n-spin size="large" :stroke="'#6B2737'" />
  </div>

  <div v-show="!loading" class="min-h-screen w-full relative overflow-hidden" :style="{
    background: 'radial-gradient(ellipse at 20% 10%, #FBF6EC 0%, #F3E9D6 45%, #E8D9BF 100%)',
    fontFamily: '\'Cormorant Garamond\', \'Noto Serif TC\', serif',
    color: '#2A1E17',
  }">
    <!-- Grain / paper texture -->
    <div class="absolute inset-0 pointer-events-none opacity-[0.18] mix-blend-multiply" :style="{
    ...
    <div class="hidden md:block absolute top-6 left-6 w-24 h-24 pointer-events-none opacity-60"
      :style="{ borderTop: '1px solid #B08D57', borderLeft: '1px solid #B08D57' }" />
    <div class="hidden md:block absolute top-6 right-6 w-24 h-24 pointer-events-none opacity-60"
      :style="{ borderTop: '1px solid #B08D57', borderRight: '1px solid #B08D57' }" />
    <div class="hidden md:block absolute bottom-6 left-6 w-24 h-24 pointer-events-none opacity-60"
      :style="{ borderBottom: '1px solid #B08D57', borderLeft: '1px solid #B08D57' }" />
    <div class="hidden md:block absolute bottom-6 right-6 w-24 h-24 pointer-events-none opacity-60"
      :style="{ borderBottom: '1px solid #B08D57', borderRight: '1px solid #B08D57' }" />

    <main
      class="relative max-w-5xl mx-auto px-5 md:px-8 py-4 md:py-20 min-h-screen flex flex-col items-center justify-start md:justify-center">
      <!-- Eyebrow -->
      <div class="flex items-center gap-3 md:gap-4 mb-3 md:mb-8 opacity-0"
        style="animation: weddingFadeUp 1s ease-out 0.05s forwards">
        <span class="block w-8 md:w-12 h-px" :style="{ background: '#B08D57' }" />
        <span class="text-[0.6rem] md:text-[0.7rem] tracking-[0.4em] md:tracking-[0.5em] uppercase"
      ...
      <h1 class="text-center opacity-0 px-2" :style="{
      ...
      <div class="my-3 md:my-8 flex items-center gap-3 opacity-0"
        style="animation: weddingFadeUp 1s ease-out 0.35s forwards">
        <span class="block w-12 md:w-20 h-px"
          :style="{ background: 'linear-gradient(to right, transparent, #B08D57)' }" />
        <svg width="12" height="12" viewBox="0 0 14 14">
          <path d="M7 0 L8.5 5.5 L14 7 L8.5 8.5 L7 14 L5.5 8.5 L0 7 L5.5 5.5 Z" fill="#B08D57" />
        </svg>
        <span class="block w-12 md:w-20 h-px"
          :style="{ background: 'linear-gradient(to left, transparent, #B08D57)' }" />
      </div>

      <!-- Framed photograph -->
      <a :href="isOpen ? weddingUrl : undefined" :target="isOpen ? '_blank' : undefined" rel="noopener"
        class="block relative group opacity-0 cursor-pointer" @click="handleLiveClick" :style="{
          width: 'min(440px, 78vw)',
          maxHeight: '46vh',
          animation: 'weddingFadeUp 1.2s ease-out 0.5s forwards',
        }">
        <!-- Gold frame -->
        <div class="absolute -inset-2 md:-inset-3 pointer-events-none transition-all duration-700 group-hover:-inset-4"
          :style="{
            border: '1px solid #B08D57',
            background: 'linear-gradient(135deg, rgba(176,141,87,0.08), transparent 40%, rgba(176,141,87,0.08))',
          }" />
        <div class="absolute -inset-1 pointer-events-none" :style="{ border: '1px solid rgba(176,141,87,0.4)' }" />
      ...
      <!-- CTA -->
      <div class="mt-6 md:mt-12 opacity-0" style="animation: weddingFadeUp 1s ease-out 0.7s forwards">
        <n-button tag="a" :href="isOpen ? (weddingUrl || undefined) : undefined" :target="isOpen ? '_blank' : undefined"
          rel="noopener" :disabled="!weddingUrl" size="large" @click="handleLiveClick" class="wedding-cta" :style="{
            '--n-color': '#6B2737',
